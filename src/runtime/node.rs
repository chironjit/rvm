...
impl Runtime for NodeRuntime {
    ...
    fn remove(&self, version: Option<&str>) -> Result<()> {
        match version {
            ...
            None => {
                ...
                display_step(&format!(
                    "Found {} Node.js versions to remove",
                ...
                display_success(&format!(
                    "Removed all {} Node.js versions successfully",
                    installed_versions.len()
                ));
            }
        }
    ...
    fn update(&self) -> Result<()> {
        ...
        display_success(&format!(
            "Successfully updated Node.js to {}",
            latest_version
        ));
        Ok(())
    }
    ...
    fn prune(&self, keep_version: &str) -> Result<()> {
        display_step(&format!(
            "Pruning Node.js versions (keeping {})",
            keep_version
        ));
    ...
    fn set_default(&self, version: &str) -> Result<()> {
        ...
        display_success(&format!(
            "Node.js {} is now the default version",
            resolved_version
        ));
        Ok(())
    }
    ...
    fn use_version(&self, version: &str) -> Result<()> {
        display_step(&format!(
            "Switching to Node.js {} for current session",
            version
        ));
        ...
        display_success(&format!(
            "Node.js {} is now active in current session",
            resolved_version
        ));
    ...
    fn list_available(&self) -> Result<Vec<String>> {
        use crate::utils::version::{
            VersionInfo, detect_channel, extract_major_minor, group_versions_by_channel,
        };
