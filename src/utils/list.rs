...
pub fn handle_list_command(args: &[String]) -> Result<()> {
    ...
    match args[0].as_str() {
        "runtimes" => match runtime::list_supported_runtimes() {
            Ok(runtimes) => {
        ...
        _ => {
            println!(
                "Unknown list command. To use list, you can either use available or installed:\n"
            );
            println!(
                "rvm list available <runtime> - lists all available runtime versions in the public repository"
            );
